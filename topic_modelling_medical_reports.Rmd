...
# Text preprocessing
We commence by loading our data compiled in the text file titled: \textcolor{red}{\textit{patient\_data.txt}} in R studio. A snapshot of the data is as seen in Figure \@ref(fig:snapshot).
...
**Note**: The full text pre-processing code is covered in \textcolor{red}{\textit{data\_preprocessing.R}} and the data is in \textcolor{red}{\textit{patient\_data.txt}} and/ or \textcolor{red}{\textit{patient\_data.csv}}.
...
## Packages
For simplicity, we are going to focus on the \textcolor{blue}{\textit{signs and symptoms}} column for each patient (row). We are going to perform topic modelling only on this column. Let's start by loading necessary packages:
...
```{r wordcloud, fig.cap="Word Cloud", out.width='60%', dpi=120}
...
beta[1:5, 1:6]
```
For every patient we have a probability distribution of the topics, *theta*. The dimensions of *theta* are 50 patients against each topic. The *theta* probabilities for each patient on the associated topic have been saved in \textcolor{red}{\textit{theta\_probabilities.csv}}.
...
\textcolor{blue}{\textit{muscle cramps, diarrhea, weightloss, swollen lymphnodes, low blood pressure}} 

highly linked to cholera. Chances are high that the first patient (patient 4) had cholera.
...
Finally, we provide a view on the topics in the patients’ records over time. Mean topic proportions per year of all patients’ sign and symptoms from the year 2010-2020 were We aggregated. These aggregated topic proportions can then be visualized, e.g. as a bar plot, Fig.\@ref(fig:timeseriesplot)
...
ggplot(vizDataFrame, aes(x=year, y=value, fill=variable)) + 
  geom_bar(stat = "identity") + 
  ylab("proportion") + 
  # This line forces integer breaks
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
    legend.position = "bottom")
```
It appears the topic on \textcolor{blue}{\textit{sweats, chills, increased thirst, muscle pain and nausea}}  which could be signs and symptoms of diabetics were on a surge at the beginning and the end of the time frame considered. There were no patients suffering from the five diseases of concern visiting the institution in 2017. 
